  ...
  useEffect(() => {
    getMyIntroductions(currentUserId).then(data => {
      if (Array.isArray(data)) { // 데이터가 배열인지 확인
        ...
        setResumes([]); // 데이터가 배열이 아닌 경우 빈 배열로 설정
      }
    }).catch(error => {
      console.error('Failed to fetch resumes:', error);
      setResumes([]); // 오류가 발생한 경우 빈 배열로 설정
    });
  }, [currentUserId]);

  // 문항 추가 (프론트: question, 백: section)
  const addQuestion = async () => {
    if (newQuestion.trim() !== "") {
      const updatedResumes = resumes.find(r => r.id === currentResumeId);
      if (!updatedResumes) {
        console.error('No resume found with the current ID');
        return; // 또는 적절한 오류 처리 로직을 추가합니다.
      }
    ...
    <>
      <div className="resume-container">
        ...
        <div className="resume-tabs">
          {Array.isArray(resumes) && resumes.map(resume => ( // resumes가 배열인지 확인
        ...
        {currentResumeId && (
          <>
            ...
            <div className="questions-list">
              {Array.isArray(resumes) && resumes.find(r => r.id === currentResumeId)?.questions.map((q, index) => ( // resumes가 배열인지 확인
